...
Revision **L3-01-r2**, accessed 2026-09-13. This is the single compact source/action record for the producer. Interpret with `research.md`; not an independent audit.
...
| [THUDM/LongBench](https://github.com/THUDM/LongBench) | Parent inspected retained original [LongBench/README.md](https://github.com/THUDM/LongBench/blob/2e00731f8d0bff23dc4325161044d0ed8af94c1e/LongBench/README.md), which links 2308.14508 and describes 21 tasks/six categories, and [LongBench/pred.py](https://github.com/THUDM/LongBench/blob/2e00731f8d0bff23dc4325161044d0ed8af94c1e/LongBench/pred.py). In `get_pred`, excess `tokenized_prompt` is reduced to its first/last halves before `build_chat`; later `model.generate` uses `do_sample=False`, `num_beams=1`. | Pinned current commit `2e00731f8d0bff23dc4325161044d0ed8af94c1e`; original benchmark retained code, not proven historical paper-execution bytes. Parent fetched/inspected both files; no execution. Current root v2 instructions remain distinct. |
...
| 18 | [LongBench subdirectory raw README attempt](https://raw.githubusercontent.com/THUDM/LongBench/main/LongBench/README.md) | Internal error in r1; later parent actions 30–31 establish the retained original path |
...
The original producer ledger records 27 source actions; parent actions 28–32 below bring the aggregate to 32; four search queries are separately listed in `research.md`. Repeated representations are not additional studies. All source transformations used text extraction/HTML normalization and paraphrase. No replication, source-code test, exhaustive integrity comparison, current leaderboard audit or specialist approval occurred.

## r2 parent follow-up source actions

Parent supplied these actually performed GitHub-connector reads on 2026-09-13. Producer incorporated their evidence without reacquisition; the earlier failed web routes remain recorded.

| Action | Exact target / operation | Evidence supplied by parent |
|---|---|---|
| 28 | `THUDM/LongBench` root contents | Repository structure discovery |
| 29 | `THUDM/LongBench` main ref | Resolved commit `2e00731f8d0bff23dc4325161044d0ed8af94c1e` |
| 30 | `LongBench/` subtree at that commit | Retained original benchmark files located |
| 31 | [Pinned original README](https://github.com/THUDM/LongBench/blob/2e00731f8d0bff23dc4325161044d0ed8af94c1e/LongBench/README.md), full file | Original paper link and task/category identity |
| 32 | [Pinned original prediction code](https://github.com/THUDM/LongBench/blob/2e00731f8d0bff23dc4325161044d0ed8af94c1e/LongBench/pred.py), full file, `get_pred` | Middle truncation before chat formatting and greedy generation settings |

G3 repaired at r2; this does not establish historical execution identity. Parent also corrected C1 against RULER v3 Table 3: Gemini stays above the operational threshold through the maximum tested 128K; the “>128K” table label is not an observation beyond 128K. Prior producer files are preserved under `revisions/r1/`; four source actions remain reserved for final audit.
