...
Revision **L3-01-r1**, 2026-09-13. **Proposal only: no dataset built, annotations commissioned, model calls made, benchmark run, or result observed.** This is the downstream application experiment, separate from the parent's fixed-input audit of this report.
...
Pin executable/data/model/prompt/retriever versions before execution. Resolve the original LongBench implementation location and select original RULER/Lost protocols explicitly if using their auxiliary diagnostics; current repository defaults may describe later benchmark generations. Do not execute installation commands or spend money under this proposal's authority.
