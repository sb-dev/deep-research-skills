...
21. public README conformance with bootstrap state kept outside the public product surface;
22. an open-source repository scaffold design;
23. one implemented and proven end-to-end core research vertical;
24. implemented progressive example coverage and representative Extension Packs;
25. Agent Skills installation and selective-installation contracts;
26. local validation and clean external installation smoke tests;
27. optional Pactwright integration kept outside core research semantics;
28. cross-domain abstraction candidates only after implementation evidence;
29. evidence-based maturity and publication gates.
...
# 22. Stage 16 — Public README Conformance Repair

## Purpose

Repair the public README so it follows the Production Skills public-surface contract without rewriting the completed Stage 15 evidence record.

The root README is a public product and onboarding surface. Bootstrap execution state, maturity evidence, verification receipts and remaining-stage bookkeeping belong under `docs/research-logs/`, not in public product copy.

## Required behaviour

Preserve the accepted Deep Research public structure while removing internal bootstrap mechanics.

The README must retain:
...
Do not expose as public product copy:
...
A genuine user-facing limitation may be stated in product terms when needed, but do not narrate the bootstrap schedule.

Keep the complete Level 1 quick-start prompt inline. Until stable public example paths exist, the fifteen example identities may remain plain public navigation rather than linking users into research logs.

## Exit

The root README is product-facing and later scaffold work can preserve it rather than redesign it around current bootstrap state.

---

# 23. Stage 17 — Cross-Project Review
...
As part of this review, verify that the repaired public README remains product-facing, keeps the complete quick start and 5 × 3 progression, and does not reintroduce bootstrap-state narration from sibling implementations.

## Exit

Useful family patterns are reused without making Deep Research conform to another domain's workflow or weakening the public README contract.

---

# 24. Stage 18 — Scaffold the Repository and Preserve the Public README
...
Adopt the repaired public README. Scaffold-time README changes are limited to mechanical publication work such as:

```text
rebase repository-relative links
link the selected repository licence and contribution guide
point the fifteen examples to stable public example paths
correct factual repository paths created by the scaffold
```

Do not independently change positioning, the complete Level 1 prompt, the 5 × 3 progression, substantive skill sections or product boundaries. Do not add bootstrap/maturity narration.

Create stable public example surfaces for the fifteen accepted primary examples. They may initially contain the accepted prompt, production problem, expected outputs and evaluation contract without pretending an example has already been executed.

Add deterministic checks for required README sections/order, complete inline quick start, exact 5 × 3 progression, public example-link closure, substantive core-skill sections, canonical documentation links and absence of bootstrap-process leakage.

## Exit

A contributor can understand where production logic, examples, packs and evaluations belong, and the scaffold preserves the accepted public README rather than redesigning it around implementation state.

---

# 25. Stage 19 — Implement and Prove the Core Vertical

Implement the minimum production capability required to execute one realistic end-to-end Deep Research task through the actual source checkout.

Use the accepted Level 1 quick start as the first vertical unless implementation evidence proves another selected Level 1 case is a better bounded proof.

The vertical should exercise:
...
→ smallest responsible repair when needed
→ focused re-evaluation
```

Use real inspected evidence. Do not substitute plans, prompts, mock results or verifier-only evidence for actual research execution.

Preserve the original failed evaluation and the corrected revision if a defect is found. A local repair should not silently overwrite the evidence that demonstrated the failure.

This stage proves one bounded vertical only. It does not establish all-example, pack, installation, benchmarked or mature status.

## Exit

The source repository can run one real producer/evaluator research vertical, retain its evidence, detect a meaningful defect and complete a bounded repair without unnecessary regeneration.

---

# 26. Stage 20 — Expand Progressive Coverage and Extension Packs

Expand from the proven core vertical to the selected progressive examples and representative Extension Packs.

Execute the accepted primary examples rather than merely copying their prompts into public paths.

Across the progression preserve the accepted coverage of:

```text
source classes
acquisition paths
fresh and stable evidence
web / PDF / code / data / GitHub evidence
source independence
contradictions
uncertainty
citation / provenance
...
For every implemented Extension Pack prove:

```text
core works without the pack
pack activation is explicit
core + pack materially changes the intended research behaviour
explicit project instructions outrank pack defaults
pack-aware evaluation recognises the specialisation
core-vs-pack comparison is fair and uses equivalent task conditions
pack authoring can create or revise a valid pack when required by the accepted architecture
```

Do not claim general provider or installation compatibility from source-checkout example execution.

## Exit

The repository demonstrates progressive research capability and real pack specialisation rather than one favoured research pattern.
...
The exact skill names are those accepted by the implemented core skill architecture.
...
evaluator-only installation
pack-creator-only installation
update and removal behaviour where part of the supported contract
```

## Exit

The intended installation contract is documented and testable from the actual production packages.

---

# 28. Stage 22 — Local Validation and Public README Conformance
...
public example links resolve
all core skills have substantive sections
installation / packs / execution / evaluation / documentation / boundary / contribution / licence surfaces exist
local links resolve
bootstrap-stage, branch, SHA, scaffold-status, maturity and not-run bookkeeping do not leak into public product copy
```

These checks validate structure and leakage; they do not prove research quality or unsupported product claims.

## Exit

The source repository is internally coherent and the public README remains a product surface after implementation work.

---

# 29. Stage 23 — Publish, Clean External Install Smoke Test and Claims Reconciliation
...
8. test at least one implemented pack.

Before publication, reconcile public README claims against actual evidence:
...
unsupported claim required for publication
→ complete the owning work or block publication

unsupported non-essential claim
→ remove it
```

Do not solve claim/evidence mismatches by exposing bootstrap-stage bookkeeping in the public README.

## Exit

The repository works as an external Agent Skills product and public claims match actual implementation and validation evidence.
...
# 32. Research-Specific Acceptance Gates
...
- one actual end-to-end producer/evaluator vertical has been proven;
- 15 primary progressive examples exist with exact prompts and are executed where required by the progressive-coverage gate;
- six canonical specification responsibilities exist;
- the README is product-facing and reconciled against actual implementation at publication;
- skills are self-contained;
- local validation passes;
- clean external installation passes;
- quality claims are backed by measured evidence or explicitly omitted/narrowed before publication.
